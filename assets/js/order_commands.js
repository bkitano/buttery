// if in beta, use "http://buttery-1-bkitano.c9users.io"
        ...
        // var socket = io.connect('https://buttery-1-bkitano.c9users.io/morse');
        ...
        socket.on('order-to-client', function(data) {
            var $order = "<tr id=" + data.id + "> <td class='name'>" + data.name + "</td> <td class='items'>"+data.order+"</td> </tr>";
        ...
        socket.on('marked-complete-to-clients', function(data) {
            ...
            var order = "<tr id=" + data.order.id + "> <td class='name'>" + data.order.name + "</td> <td class='items'>"+data.order.items+"</td> </tr>";            
            console.log(order);
            $('#completed-orders-list').append(order);
